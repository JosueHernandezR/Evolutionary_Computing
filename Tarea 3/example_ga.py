...
# Corrección del nombre de la biblioteca.
from tkinter import *
...
for i in range(0, int(N_chromosomes)):
...
def decode_chromosome(chromosome):
    ...
    # in Python3, conversion to float is not needed
    return a+(b-a)*float(value)/(N_chains-1)

# Modificar rastrigin y ackley 
def f(x):
...
def evaluate_chromosomes():
    ...
    for p in range(int(N_chromosomes)):
...
def nextgeneration():
    ...
    for i in range(0, int((N_chromosomes-2)/2)):
        ...
        o1 = F0[p1][0:int(crossover_point)]
        o1.extend(F0[p2][int(crossover_point):L_chromosome])
        o2 = F0[p2][0:int(crossover_point)]
        o2.extend(F0[p1][int(crossover_point):L_chromosome])
...
def graph_population(F, mycanvas, escalax, escalay, xcentro, ycentro, color):
    for chromosome in F:
        ...
        mycanvas.create_line(xcentro+x*escalax, ycentro-10*escalay,
                             xcentro+x*escalax, ycentro+10*escalay, fill=color)
